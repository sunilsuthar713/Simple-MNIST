...
# Contains all the images and labels in the MNIST data set
mnist_data = input_data.read_data_sets('MSIST_data', one_hot=True)
...
# y = Wx + b
# Input to the graph, take in any number of images (784 element pixel arrays)
x_input = tf.placeholder(dtype=tf.float32, shape=[None, 784], name='x_input')
# Weight to be multiplied by input
W = tf.Variable(initial_value=tf.zeros(shape=[784, 10]), name='W')
# Biased to be added to weights * inputs
b = tf.Variable(initial_value=tf.zeros(shape=[10]), name='b')
# Actual model prediction based on input and current value of W and b
...
# Train the model by fetching batches of 100 images
...
# Measure accuracy by comparing the predicted values to the correct value
